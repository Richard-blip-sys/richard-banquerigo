...
if(isset($_POST['add_faculty'])) {
    ...
    try {
        ...
        $query = "INSERT INTO users (username, password_hash, email, first_name, last_name, user_type, email_verified) 
                  VALUES (:username, :password_hash, :email, :first_name, :last_name, 'faculty', 1)";
...
<body>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if($faculty['pending_requests'] > 0): ?>
                                            <span class="status-badge status-pending"><?php echo $faculty['pending_requests']; ?></span>
                                        <?php else: ?>
                                    ...
                                    <td class="action-buttons">
                                        <button class="edit-btn" onclick='openEditModal(<?php echo json_encode($faculty); ?>)'>✏️ Edit</button>
                                        <button class="btn-secondary" style="padding: 8px 15px; font-size: 14px;" onclick="openResetModal(<?php echo $faculty['user_id']; ?>, '<?php echo htmlspecialchars($faculty['username']); ?>')">🔑 Reset</button>
                                        <button class="delete-btn" onclick="openDeleteModal(<?php echo $faculty['user_id']; ?>, '<?php echo htmlspecialchars($faculty['username']); ?>', <?php echo $faculty['total_enrollments']; ?>)">🗑️ Delete</button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
    ...
    <div id="deleteModal" class="modal">
        <div class="modal-content modern-modal small-modal">
            <div class="modal-header">
                <h2>🗑️ Delete Faculty Account</h2>
            </div>
            <form method="POST">
        ...
        function openDeleteModal(userId, username, enrollments) {
            ...
            if(enrollments > 0) {
                document.getElementById('deleteMessage').innerHTML = 
                    '<strong>⚠️ Warning:</strong> Cannot delete faculty <strong>' + username + '</strong> because they have ' + 
